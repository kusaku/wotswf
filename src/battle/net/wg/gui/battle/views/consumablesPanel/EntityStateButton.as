...
public class EntityStateButton extends BattleButton implements IEntityStateButton {
    ...
    private const FIRST_MAN_POSTFIX:String = "1";

    private const SECOND_MAN_POSTFIX:String = "2";

    private const ENTITY_STATE_POSTFIX:String = "_orange";
    ...
    private function showIcon(param1:Sprite, param2:String):void {
        ...
        var _loc3_:String = this._entityState == BATTLE_ITEM_STATES.NORMAL ? Values.EMPTY_STR : this.ENTITY_STATE_POSTFIX;
        if (param2 == RolesState.LOADER + this.FIRST_MAN_POSTFIX || param2 == RolesState.LOADER + this.SECOND_MAN_POSTFIX) {
            _loc4_ = RolesState.LOADER + _loc3_;
        }
        else if (param2 == RolesState.GUNNER + this.FIRST_MAN_POSTFIX || param2 == RolesState.GUNNER + this.SECOND_MAN_POSTFIX) {
            _loc4_ = RolesState.GUNNER + _loc3_;
        }
        else if (param2 == RolesState.RADIOMAN + this.FIRST_MAN_POSTFIX || param2 == RolesState.RADIOMAN + this.SECOND_MAN_POSTFIX) {
            _loc4_ = RolesState.RADIOMAN + _loc3_;
        }
        else {
            _loc4_ = param2 + _loc3_;
        }
        this._atlasManager.drawGraphics(AtlasConstants.BATTLE_ATLAS, _loc4_, param1.graphics);
    }
