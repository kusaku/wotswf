...
public class BattleMessengerActionContainer extends Sprite implements IDisposable, IClickButtonHandler {
    ...
    private var _squadIconPadding:Point;

    private var _disabledStateOffset:Point;

    private var _vehicleID:Number = -1;

    private var _isShowContainer:Boolean = false;

    private const BG_PADDING_BY_Y:int = 5;

    private const BLACK_LIST_BTN_X_POS:int = 1;

    private const BLACK_LIST_BTN_Y_POS:int = 2;

    private const BG_SHAPE_TOP_PADDING:int = -2;

    public function BattleMessengerActionContainer() {
        this._atlasManager = App.atlasMgr;
        this._muteBtnImageMap = new Dictionary();
        this._squadIconPadding = new Point(7, 1);
        this._disabledStateOffset = new Point(-9, 0);
        super();
        ...
        this._muteBtnImageMap[BATTLE_MESSAGES_CONSTS.ADD_IN_BLACKLIST] = {
            "defaultState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_OFF_NORMAL)],
            "hoverState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_OFF_HOVER)],
            "disableState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_OFF_DISABLEL)]
        };
        this._muteBtnImageMap[BATTLE_MESSAGES_CONSTS.REMOVE_FROM_BLACKLIST] = {
            "defaultState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_ON_NORMAL)],
            "hoverState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_ON_HOVER)],
            "disableState": [new BattleStateBtnSettings(BattleAtlasItem.BUTTON_TOXIC_CHAT_ON_DISABLEL)]
        };
        this.buttonsConfig();
    }

    public function dispose():void {
        this._squadIconPadding = null;
        this._disabledStateOffset = null;
    ...
    public function drawTestGraphics(param1:BattleMessengerToxicVO, param2:Boolean, param3:Function, param4:int, param5:int, param6:int):void {
        ...
        this._bg.x = 0;
        this._bg.y = this.BG_SHAPE_TOP_PADDING;
        this._bg.width = param5;
        this._bg.height = param6 + this.BG_PADDING_BY_Y;
        this._blackListButton.x = this.BLACK_LIST_BTN_X_POS;
        this._blackListButton.y = this.BLACK_LIST_BTN_Y_POS;
    ...
    private function updateCenterPositionByHeight():void {
        this._blackListButton.y = (this._bg.height - this._blackListButton.actualHeight >> 1) + this.BG_SHAPE_TOP_PADDING;
    }
    ...
    private function setAtlasProperties(param1:BattleStateButton):void {
        param1.atlasName = AtlasConstants.BATTLE_ATLAS;
