...
public class TeamMemberRenderer extends TeamMemberRendererBase {
    ...
    override protected function showToolTips():void {
        ...
        if (model.accID == -1) {
            ...
            _loc1_ = this.toolTipStr == Values.EMPTY_STR ? TOOLTIPS.MEMBERS_VEHICLELEVELLIMITS_BODY : this.toolTipStr;
        }
        else {
            _loc1_ = getToolTipData();
        }
    ...
    override protected function updateAfterStateChange():void {
        ...
        vehicleNameField.htmlText = !!model ? model.vShortName : Values.EMPTY_STR;
        this.numberField.text = !!model ? String(model.orderNumber) : Values.EMPTY_STR;
        var _loc1_:Number = !!model ? Number(model.getCurrentColor()) : Number(Number.NaN);
        if (!isNaN(_loc1_)) {
