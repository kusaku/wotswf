    ...
    private function validateAwardsData(param1:Vector.<AwardsItemVO>):void {
        ...
        for each(_loc2_ in param1) {
            if (StringUtils.isNotEmpty(_loc2_.itemSource) && _loc2_.boosterVO != null) {
                App.utils.asserter.assert(false, "AwardsBlockVO.awards can not have itemSource and boosterVO in one time!");
                return;
            }
        }
    }
    ...
    private function onLastRendererReadyHandler(param1:AwardWindowEvent):void {
        ...
        if (this._awardsLength > 0) {
            ...
            while (_loc6_ < this._awardsLength) {
                _loc2_ = _loc2_ + this._awardsRenderers[_loc6_].actualWidth;
                _loc11_ = _loc2_ + MIN_BLOCKS_GAP * _loc6_;
                if (_loc11_ > _loc4_) {
                    DebugUtils.LOG_WARNING("Width of awards is too much to be placed into award ribbon!");
                    _loc5_ = _loc6_;
                    break;
                }
                _loc6_++;
            ...
            while (_loc6_ < _loc5_) {
                ...
                _loc8_ = _loc8_ + (_loc10_.actualWidth + _loc7_);
                _loc6_++;
            }
        }
    }
