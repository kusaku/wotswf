...
public class TeamStatsControllerAbstract implements IDisposable {
    ...
    public function update(param1:BattleResultsVO):void {
        this.setupColumns(this._columns);
    ...
    protected function getColumnIds():Vector.<String> {
    ...
    protected function initColumnsData():void {
        ...
        this._columnWidth[ColumnConstants.PLAYER] = ColumnConstants.DEFAULT_PLAYER_COLUMN_WIDTH;
    ...
    private function setupColumns(param1:ColumnCollection):void {
        var _loc3_:String = null;
        var _loc4_:ColumnData = null;
        var _loc5_:Boolean = false;
        var _loc6_:String = null;
        this.initColumnsData();
        var _loc2_:Vector.<String> = this.getColumnIds();
        for each(_loc3_ in _loc2_) {
            _loc4_ = param1.add(_loc3_, this._columnTooltip[_loc3_], RES_ICONS.maps_icons_buttons_tab_sort_button(_loc3_ + ".png"), this._columnWidth[_loc3_]);
            if (_loc3_ in this._sortingKey) {
                _loc5_ = _loc3_ in this._sortingIsNumeric ? Boolean(this._sortingIsNumeric[_loc3_]) : true;
                _loc6_ = _loc3_ in this._sortingDirection ? this._sortingDirection[_loc3_] : SortingInfo.DESCENDING_SORT;
                _loc4_.setSorting(this._sortingKey[_loc3_], _loc5_, _loc6_);
            }
        }
