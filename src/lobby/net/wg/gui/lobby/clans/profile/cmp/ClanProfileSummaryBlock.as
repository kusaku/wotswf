...
public class ClanProfileSummaryBlock extends UIComponentEx implements IClanProfileSummaryBlock {
    ...
    private static const Y_POS_WITHOUT_HEADER:int = 40;
    ...
    override protected function draw():void {
        super.draw();
        if (isInvalid(InvalidationType.SIZE) && this._isShowHeader) {
    ...
    private function makeActivatedBlocks(param1:ClanProfileSummaryBlockVO):void {
        var _loc5_:ClanProfileStatsLineVO = null;
        var _loc6_:IDashLineTextItem = null;
        var _loc2_:int = param1.statBlocks.length;
        var _loc3_:int = !!this._isShowHeader ? int(Y_POS_WITHOUT_HEADER) : 0;
        var _loc4_:int = 0;
        while (_loc4_ < _loc2_) {
            _loc5_ = param1.getBlockAt(_loc4_);
            _loc6_ = this._utils.getComponent(Linkages.DASH_LINE_TEXT_ITEM, IDashLineTextItem);
            this.addChild(_loc6_ as DisplayObject);
            this.updateItemPosition(_loc6_, _loc3_);
            _loc3_ = _loc3_ + RENDER_PADDING;
            this.updateBlockProperties(_loc6_, _loc5_);
            this._dynItems.push(_loc6_);
            _loc4_++;
        }
        this._dynHeight = _loc3_;
    }
