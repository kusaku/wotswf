...
public class SlidingScrollingList extends ScrollingList {
    ...
    private static const SB_PADDING:int = 5;
    ...
    override protected function drawLayout():void {
        ...
        if (this._totalHeight < this.height) {
            ...
            if (this._totalHeight > 0) {
                this._totalHeight = this._totalHeight + DOWN_MARGIN;
            }
            this.drawScrollBar();
        }
        if (this._totalHeight == 0) {
    ...
    override protected function drawScrollBar():void {
        ...
        _scrollBar.x = _width - _scrollBar.width - SB_PADDING;
        _scrollBar.y = SB_PADDING;
        _scrollBar.height = availableHeight - (SB_PADDING << 1);
        _scrollBar.validateNow();
    }
    ...
    override protected function scrollList(param1:int):void {
        ...
        _scrollBar.position = _scrollBar.position - param1 * this._stepSize;
        _scrollBar.validateNow();
    }
    ...
    override public function get availableWidth():Number {
        return Math.round(_width) - (margin << 1) - (!!_autoScrollBar ? Math.round(_scrollBar.width + SB_PADDING) : 0);
    }
