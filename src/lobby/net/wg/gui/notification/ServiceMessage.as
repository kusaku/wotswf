...
public class ServiceMessage extends UIComponentEx {
    ...
    private function createButtonsGroup(param1:MessageInfoVO):void {
        ...
        while (_loc8_ < _loc3_) {
            ...
            _loc7_ = _loc5_.type;
            _loc9_ = ButtonType.getLinkageByType(_loc7_);
            if (_loc9_ != null) {
                _loc6_ = App.utils.classFactory.getComponent(_loc9_, DisplayObject);
                if (_loc6_ == null) {
                    DebugUtils.LOG_ERROR(CANT_CREATE_BUTTON, _loc9_);
                }
                else {
                    this.createButton(SoundButtonEx(_loc6_), _loc5_, param1);
                }
            }
            _loc8_++;
        }
    ...
    private function createButton(param1:SoundButtonEx, param2:ButtonVO, param3:MessageInfoVO):void {
        ...
        this._buttonsGroup.addChild(param1);
        param1.focusable = false;
        param1.validateNow();
        param1.addEventListener(ButtonEvent.CLICK, this.onBtnClickHandler, false, 0, true);
    }
