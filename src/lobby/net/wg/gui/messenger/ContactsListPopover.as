...
public class ContactsListPopover extends ContactsListPopoverMeta implements IContactsListPopoverMeta {
    ...
    private static function getMoveGroupSendData(param1:ContactsGroupEvent):Object {
        return {
            ...
            "excludeParentGroup": (!!param1.excludeGroup.parent ? param1.excludeGroup.parent.id : null),
            "targetParentGroup": (!!param1.targetGroup.parent ? param1.targetGroup.parent.id : null)
        };
    }
        ...
        if (isInvalid(FOCUS_INVALID)) {
            if (this.viewStack.currentView) {
                if (this._focusInited) {
                    setFocus(IFocusContainer(this.viewStack.currentView).getComponentForFocus());
                }
    ...
    public function as_removeGroup(param1:String):void {
        var _loc2_:IUpdatable = IUpdatable(this.viewStack.show(CONTACTS_ALIASES.GROUP_DELETE_VIEW_LINKAGE));
    ...
    private function manageOperation(param1:String, param2:String, param3:Object = null):void {
        ...
        var _loc4_:IUpdatable = IUpdatable(this.viewStack.show(param1));
        if (param3) {
    ...
    private function checkGroupBtn():void {
        this.addGroupButton.enabled = !!this._initData ? Boolean(this._initData.isGroupSupported) : false;
    ...
    private function onAddContactButtonClickHandler(param1:ButtonEvent):void {
        this.viewStack.show(CONTACTS_ALIASES.FIND_CONTACT_VIEW_LINKAGE);
    }
    ...
    private function onSettingsButtonClickHandler(param1:ButtonEvent):void {
        this.viewStack.show(CONTACTS_ALIASES.CONTACTS_SETTINGS_VIEW_LINKAGE);
    }

    private function onViewStackOnCancelClickHandler(param1:Event):void {
        param1.stopImmediatePropagation();
        if (this.viewStack.currentLinkage) {
            unregisterComponent(this._linkageUtils.getAliasByLinkage(this.viewStack.currentLinkage));
            invalidate(FOCUS_INVALID);
        }
    }

    private function onViewStackViewShowHandler(param1:ViewStackEvent):void {
        var _loc2_:String = this._linkageUtils.getAliasByLinkage(param1.linkage);
