...
public class TooltipEnvironment extends ToolTipSpecial {
    ...
    override protected function redraw():void {
        ...
        topPosition = topPosition + (this.titleTF.textHeight + TEXTFIELD_PADDING);
        if (this.icon.visible) {
            topPosition = topPosition + TITLE_ICON_GAP;
            this.icon.source = _loc1_.icon;
            this.icon.y = topPosition | 0;
            this.icon.x = _loc2_ | 0;
            topPosition = topPosition + (this.icon.originalHeight + ICON_TEXT_GAP);
        }
        else {
            topPosition = topPosition + TITLE_TEXT_GAP;
        }
