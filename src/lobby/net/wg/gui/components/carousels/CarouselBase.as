...
public class CarouselBase extends CoreList implements IDraggable {
    ...
    override protected function drawRenderers(param1:Number):void {
        ...
        var _loc6_:* = _renderers.length > this.countVisibleSlots;
    ...
    private function renderersMouseEnabled(param1:Boolean = false):void {
        var _loc2_:int = !!_renderers ? int(_renderers.length) : 0;
        var _loc3_:int = 0;
        while (_loc3_ < _loc2_) {
    ...
    private function slidingFn(param1:Object):void {
        ...
        this._lastDx = this._lastDx + -this._lastDx / this._slideSelfAcceleratorDynamic;
        this.currentFirstRenderer = this.getCurrentFirstRendererOnAnim();
        if (_loc2_ > param1.scopeDefPosition) {
            container.x = container.x + (param1.scopeDefPosition - container.x + this._lastDx) / SLIDE_TO_POS_ACCELERATOR;
            ...
            container.x = container.x + (param1.scopeDefPosition - param1.allowDragDistance - container.x + this._lastDx) / SLIDE_TO_POS_ACCELERATOR;
            this._slideSelfAcceleratorDynamic = SLIDE_TO_POS_ACCELERATOR;
    ...
    private function arrowSlide():void {
        ...
        var _loc2_:Number = NaN;
        this.startOrStopInvokingArrowSlide(true);
        this._lastDx = this._lastDx + (MAX_DX * this._sourceFactor - this._lastDx) / SLIDE_SELF_ACCELERATOR;
        if (this._sourceFactor == 0) {
            ...
            this._countFactor++;
            container.x = container.x + this._lastDx;
