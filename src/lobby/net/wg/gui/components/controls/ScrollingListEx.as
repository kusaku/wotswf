...
public class ScrollingListEx extends CoreListEx {
    ...
    protected function navigateSelection(param1:int, param2:int):void {
        if (this._selectionNavigator == null) {
            this.selectedIndex = param1;
        }
        else {
            param1 = this._selectionNavigator.navigate(param1, param2);
            if (param1 != -1) {
                this.selectedIndex = param1;
            }
        }
    ...
    public function get renderersCount():int {
        return !!_renderers ? int(_renderers.length) : -1;
    }
    ...
    override public function handleInput(param1:InputEvent):void {
        ...
        switch (_loc3_.navEquivalent) {
            case NavigationCode.UP:
                if (selectedIndex == -1) {
                    if (_loc4_) {
                        this.navigateSelection(this.scrollPosition + _totalRenderers - 1, -1);
                    }
                }
                else if (_selectedIndex > 0) {
                    if (_loc4_) {
                        this.navigateSelection(selectedIndex - 1, -1);
                    }
                }
                else if (this.wrapping != WrappingMode.STICK) {
                    if (this.wrapping == WrappingMode.WRAP) {
                        if (_loc4_) {
                            this.navigateSelection(_dataProvider.length - 1, -1);
                        }
                    }
                    else {
                        return;
            ...
            case NavigationCode.DOWN:
                if (_selectedIndex == -1) {
                    if (_loc4_) {
                        this.navigateSelection(this._scrollPosition, 1);
                    }
                }
            ...
            case NavigationCode.PAGE_UP:
                if (_loc4_) {
                    this.navigateSelection(Math.max(0, _selectedIndex - _totalRenderers), -1);
                }
                break;
            case NavigationCode.PAGE_DOWN:
                if (_loc4_) {
                    this.navigateSelection(Math.min(_dataProvider.length - 1, _selectedIndex + _totalRenderers), 1);
                }
                break;
            default:
                return;
    ...
    protected function onScrollBarChangeHandler(param1:Event):void {
