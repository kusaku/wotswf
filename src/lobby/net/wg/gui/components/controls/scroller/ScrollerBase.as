...
public class ScrollerBase extends UIComponentEx implements IScrollerBase {
    ...
    private static const INTERVAL_SPEED:Number = 16.6666666666667;
    ...
    private var _viewPort:IViewPort = null;
    ...
    override protected function draw():void {
        ...
        var _loc4_:Boolean = isInvalid(InvalidationType.SIZE);
        ...
        var _loc5_:Boolean = isInvalid(INVALIDATION_TYPE_SCROLL);
        if (_loc5_ && hasEventListener(Event.SCROLL)) {
            dispatchEvent(new Event(Event.SCROLL));
        }
        if (_loc1_ || _loc4_ || _loc5_ || _loc3_ || _loc2_) {
            this.layoutChildren();
        }
        if (isInvalid(INVALIDATION_TYPE_CLIPPING) || _loc4_) {
            this.createMask();
        }
        if (_loc4_ && hasEventListener(ScrollEvent.UPDATE_SIZE)) {
    ...
    public function scrollInDirection(param1:Number):void {
        ...
        if (this._mouseWheelDirection == DirectionMode.HORIZONTAL) {
            if (this._targetHorizontalScrollPosition > 0 && this._hasHorizontalElasticEdges) {
                _loc3_ = this._targetHorizontalScrollPosition;
            }
            else {
                _loc3_ = this._horizontalScrollPosition;
            }
            _loc4_ = this.horizontalScrollStep;
            if (_loc4_ !== _loc4_) {
                _loc4_ = this._minHorizontalScrollPosition + this._pageWidth * this._scrollConfig.mouseWheelScrollPercent;
            }
            ...
            if (_loc3_ <= this._minHorizontalScrollPosition) {
                ...
                _loc2_ = this.calculateDynamicThrowDurationFromDistance(_loc3_);
            }
            this.throwToHorizontalPosition(_loc3_, _loc2_);
        }
        else if (this._mouseWheelDirection == DirectionMode.VERTICAL) {
            if (this._targetVerticalScrollPosition > 0 && this._hasVerticalElasticEdges) {
            ...
            if (_loc6_ !== _loc6_) {
                _loc6_ = this._minVerticalScrollPosition + (this._maxVerticalScrollPosition - this._minVerticalScrollPosition) * this._scrollConfig.mouseWheelScrollPercent;
            }
            _loc5_ = _loc5_ - param1 * _loc6_;
            if (_loc5_ <= this._minVerticalScrollPosition) {
                ...
                _loc2_ = this.calculateDynamicThrowDurationFromDistance(_loc5_);
            }
    ...
    protected function refreshScrollValues():void {
        ...
        if (_loc2_ !== this._maxVerticalScrollPosition) {
            if (this._tweensVertically) {
                ...
                this.verticalScrollPosition = this._targetVerticalScrollPosition = this.clampScrollPosition(this._verticalScrollPosition, this._minVerticalScrollPosition, this._maxVerticalScrollPosition, this.verticalScrollStep, false);
            }
        }
    ...
    protected function calculateDynamicThrowDurationFromDistance(param1:Number):Number {
        return this.calculateDynamicThrowDuration(param1 * this._logDecelerationRate);
    }
    ...
    protected function isViewPortUnderPoint(param1:Number, param2:Number):Boolean {
        return param1 >= this._viewPortOffsetLeft && param1 < this._viewPortOffsetLeft + this._viewPort.visibleWidth && param2 >= this._viewPortOffsetTop && param2 < this._viewPortOffsetTop + this._viewPort.visibleHeight;
    }
    ...
    protected function completeTouchScroll():void {
        ...
        if (_loc3_ || _loc4_) {
            ...
            while (_loc8_ < this._velocityLength) {
                ...
                _loc5_ = _loc5_ + this._velocityBufferX[_loc8_] * _loc9_;
                _loc6_ = _loc6_ + this._velocityBufferY[_loc8_] * _loc9_;
    ...
    private function clampScrollPosition(param1:Number, param2:Number, param3:Number, param4:Number = NaN, param5:Boolean = true, param6:Number = 2147483647):Number {
        ...
        if (param1 < param2) {
            if (param5) {
                param1 = param1 - (param1 - param2) * (1 - this._scrollConfig.elasticity);
            ...
            if (param5) {
                param1 = param1 - (param1 - param3) * (1 - this._scrollConfig.elasticity);
    ...
    private function onDragUpdateHandler(param1:Event):void {
        ...
        if (this._isDraggingHorizontally) {
            ...
            this.horizontalScrollPosition = this.horizontalScrollPosition + (_loc11_ - this.horizontalScrollPosition) / this._scrollConfig.throwAccelerationRate;
        }
        if (this._isDraggingVertically) {
            ...
            this.verticalScrollPosition = this.verticalScrollPosition + (_loc12_ - this.verticalScrollPosition) / this._scrollConfig.throwAccelerationRate;
        }
    }
